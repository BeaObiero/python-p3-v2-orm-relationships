...
class Employee:
    ...
    def __init__(self, name, job_title, department_id,  id=None):
        ...
        self.department_id = department_id #foreign key that connects each employee to a specific department
    ...
    def instance_from_db(cls, row):
        ...
        if employee:
            ...
            employee.department_id = row[3] #update to handle new column(foreign id)
        else:
            # not in dictionary, create new instance and add to dictionary
            employee = cls(row[1], row[2],row[3]) #update to handle new column
            employee.id = row[0]
